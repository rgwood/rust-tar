...
impl Header<'_> {
    ...
    fn verify_checksum(&self) {
        ...
        if self.checksum != calculated_checksum {
            panic!(
                "Invalid checksum {} for file '{}' in tarball. We calculated {}",
                self.checksum, self.file_name, calculated_checksum
            );
        }
    }
}
...
fn main() -> std::io::Result<()> {
    ...
    if args.list_mode || args.extract_mode {
        ...
        println!(
            "Read all {} bytes from tarball successfully in {:?}",
            file_size_in_bytes,
            Instant::now().duration_since(read_start)
        );

        let iterator = TarballIterator {
            bytes: &tarball_contents[..],
            byte_offset: 0,
        };
